...
const TshirtSlider2 = () => {
  ...
  const [custTshirts, setCustTshirts] = useState([]);
  const handleProducts = async () => {
    setCustTshirts([]);
    const data = await fetchFromTag("Customised T-Shirt");
    setCustTshirts(data);
  };
  useEffect(() => {
    handleProducts();
  ...
  return (
    <div>
      <Slider {...settings}>
        {custTshirts.map((item) => (
          <div className="p-4 mb-10 lg:w-1/4 md:w-1/2">
